...
//Pin sensor de flama
//---------------------------------------------------------
const int flamaPin = 3; // Pin para el sensor de flama
int flama = 0;

//---------------------------------------------------------
//Pines del modulo relé 
//---------------------------------------------------------
...
void setup() {
  ...
  pinMode(flamaPin, INPUT);//sensor de flama

  pinMode(relePin, OUTPUT);//sensor de rele
}
...
void loop() {
  ...
  //Si la distancia es menor a 30 cm se acerca al totem
  if(distance < 30){
    avanzar();
    //Si la distancia es menor a 10 cm y esta dentro del circulo blanco lee el sensor de flama 
    if(sensorValueIzq > 400 && sensorValueDer > 400 && distance < 10){
      detener();
      flama = digitalRead(flamaPin);
      //si hay flama enciende turbina sino retrocede y gira 
      if(flama == HIGH){
